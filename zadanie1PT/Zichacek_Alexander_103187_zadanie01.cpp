/*
Meno a priezvisko: Alexander Zichacek
...
int numDigits(int value) {
	string pole = to_string(value);
	int length = 0;
	while (pole[length]!='\0') length++;

    return length; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
void print(const Date *date, const char *format) {
	...
	while (format[length]!='\0'){
		if(format[length] == 'D') std::cout << date->day;
		else if(format[length] == 'M') std::cout << date->month;
		else if(format[length] == 'Y') std::cout << date->year;
		else std::cout << format[length];
		length++;
	}
}
...
Date* create(int day, int month, int year) {
    // TODO
    Date *datum = new Date;
    datum->day = day; datum->month = month; datum->year = year;
    return datum; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
bool isInLeapYear(const Date *date) {
    // TODO
		if (date->year % 4 == 0){
			if ((date->year % 100 == 0) && (date->year % 400 != 0)) return false;
			else return true;
		}
	return false;
}
...
bool isValid(const Date *date) {
    ...
    else if((date->month == 4 || date->month == 6 || date->month == 9 || date->month == 11) && (date->day > 30)) return false;
    else if((date->month == 1 || date->month == 3 || date->month == 5 || date->month == 7 || date->month == 8 || date->month == 10 || date->month == 12) && (date->day > 31)) return false;
    else if((isInLeapYear(date) == true) && (date->month == 2) && (date->day > 29)) return false;  
	else if((isInLeapYear(date) == false) && (date->month == 2) && (date->day > 28)) return false;  
    else return true;
}
